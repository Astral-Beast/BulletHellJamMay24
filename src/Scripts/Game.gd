...
func _ready():
	...
	add_enemy(SIDE_LEFT,SIDE_RIGHT, Enums.Pathing.HOVER_ON_POINT)
	

func add_enemy(enter_side:Side, exit_side:Side, pathing_type):
	var new_foe = foe_with_path.instantiate()
	new_foe.initialize(enter_side, exit_side, pathing_type)
	add_child(new_foe)
